...
def bitrate_smo(outputs):
    ...
    for idx, scheme in enumerate(SCHEMES):
        ...
        out_str = '%s %.2f %.2f %.2f %.2f %.2f %.2f'%(labels[idx], mean_, low_, high_, mean_rebuf_, low_rebuf_, high_rebuf_)
        print(out_str)
...
def qoe_cdf(outputs):
    ...
    for idx, scheme in enumerate(SCHEMES):
        ...
        print('%s, %.2f' % (labels[idx], np.mean(mean_arr)))
    ax.set_xlabel('QoE')
    ax.set_ylabel('CDF')
    ax.set_ylim(0., 1.01)
    ax.set_xlim(0., 1.8)
